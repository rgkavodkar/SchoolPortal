class StudentCoursesController < ApplicationController
    ...
    def course_history_display
        if logged_in?
            ...
            if subset == "pending"
                @student_courses = StudentCourse.where(user_id:current_user.id, status:"pending")
            elsif subset == "enrolled"
                @student_courses = StudentCourse.where(user_id:current_user.id, status:"enrolled")
            elsif subset == "completed"
                @student_courses = StudentCourse.where(user_id:current_user.id, status:"completed")
            else
    ...
    def get_student_history
        if logged_in?
            if current_user.utype == "admin"
                ...
                @student_courses = StudentCourse.all.map{|course| course if (course.user_id == user.id)}
                @student = user
            else
